...
app.config['MYSQL_DB'] = 'r_humanos_nova'
mysql = MySQL(app)
...
@app.route('/periodo')
def periodo():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
    cursor = conn.cursor()
    cursor.execute('select Id_periodo,  descripcion from periodo order by descripcion')
    datos=cursor.fetchall()
    conn.close()
    return render_template("tabla_periodo_pago.html", peris=datos)

@app.route('/agr_periodo')
def agr_periodo():
    return render_template("agregar_periodo.html")
...
def periodo_agr():
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
        cursor = conn.cursor()
        cursor.execute('select count(*) from periodo where Descripcion = %s',(aux_descripcion))
        niveles = cursor.fetchone()
        if (niveles[0] != 0):
            error = "El Periodo de pago ya se encuentra agregado."
            return render_template("error.html", des_error=error, paginaant="/agr_periodo")
        else:
            cursor.execute('insert into periodo (Descripcion) values (%s)', (aux_descripcion))
            conn.commit()
            conn.close()
...
def ed_periodo(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
    cursor = conn.cursor()
    cursor.execute('select Id_periodo, Descripcion from periodo where Id_periodo = %s', (id))
    dato=cursor.fetchall()
    conn.close()
    return render_template("edi_periodo.html", periodos = dato[0])
...
def modifica_periodo(id):
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
        cursor = conn.cursor()
        cursor.execute('update periodo set  descripcion=%s where Id_periodo=%s', (descrip, id))
        conn.commit()
        conn.close()
    return redirect(url_for('periodo'))
...
def bo_periodo(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
    cursor = conn.cursor()
    cursor.execute('delete from periodo where Id_periodo = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('periodo'))

#-------------------------------------- tipo nomina--------------------------
@app.route('/tip_nomina')
def tip_nomina():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
    cursor = conn.cursor()
    cursor.execute('select idtipo_nomina,  descripcion from tipo_nomina order by descripcion')
    datos=cursor.fetchall()
    conn.close()
    return render_template("tabla_tip_nom.html", tipos=datos)

@app.route('/agr_tipo_nomina')
def agr_tipo_nomina():
    return render_template("agregar_tipo_nomina.html")

@app.route('/t_nom_agr', methods=['POST'])
def t_nom_agr():
    if request.method == 'POST':
        aux_descripcion = request.form['descripcion']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
        cursor = conn.cursor()
        cursor.execute('select count(*) from tipo_nomina where Descripcion = %s',(aux_descripcion))
        niveles = cursor.fetchone()
        if (niveles[0] != 0):
            ...
            cursor.execute('insert into tipo_nomina (Descripcion) values (%s)', (aux_descripcion))
            conn.commit()
            conn.close()
    return redirect(url_for('tip_nomina'))

@app.route('/ed_tipo_nomina/<string:id>')
def ed_tipo_nomina(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
    cursor = conn.cursor()
    cursor.execute('select idtipo_nomina, Descripcion from tipo_nomina where idtipo_nomina = %s', (id))
    dato=cursor.fetchall()
    conn.close()
    return render_template("editar_tipo_nomina.html", tipos = dato[0])
...
def modifica_tipo_nomina(id):
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
        cursor = conn.cursor()
        cursor.execute('update tipo_nomina set  descripcion=%s where idtipo_nomina=%s', (descrip, id))
        conn.commit()
        conn.close()
    return redirect(url_for('tip_nomina'))
...
@app.route('/bo_tip_nom/<string:id>')
def bo_tip_nom(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos_nova')
    cursor = conn.cursor()
    cursor.execute('delete from tipo_nomina where idtipo_nomina = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('tip_nomina'))
